...
def stock_path_sim(N,TT,S_0,mu,sigma2):
    ...
    W = np.sqrt(TT/N)*np.append(0,np.cumsum(np.random.normal(0,1,N)))
    #W = np.append(0,np.cumsum(np.random.normal(0,1,N)))
    S = S_0*np.exp((mu-0.5*(sigma2)**0.5)*t+(sigma2)**0.5*W)
    return S
...
BTC1["IV"]=imp
# BTC1["BS analytical"] =  [black_scholes_call(S, BTC1.loc[x]['K'], BTC1.loc[x]['T']/365, m[crypto], sigma2_y[crypto]**0.5) for x in BTC1.index]   
# test=BTC1
