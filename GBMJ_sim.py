...
for crypto in Y.keys():
    ...
    #               ,kde=True,stat="density",log_scale=True,color=sns.color_palette(),bins=1000)
...
#     J = np.append(0,np.random.poisson(lamb,N))
#     W = np.sqrt(TT/N)*np.append(0,np.cumsum(np.random.normal(0,1,N)))
#     S = S_0*np.exp((mu-0.5*(sigma2)**0.5)*t+sigma2**0.5*W+np.cumsum(J*xi))
#     return S,J*xi
...
# #imp = [implied_volatility_call(ETH1.loc[x]['Call price'],S,ETH1.loc[x]['K'],ETH1.loc[x]['T']/365,m[crypto],max_iterations=int(1e4)) for x in ETH1.index]   
